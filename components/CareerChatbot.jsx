...
function Message({ msg, userInfo }) {
  ...
  return (
    <div style={{ display: "flex", gap: 12, flexDirection: isUser ? "row-reverse" : "row", alignItems: "flex-start" }}>
      <div style={{ width: 30, height: 30, minWidth: 30, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, flexShrink: 0, background: isUser ? "rgba(139,92,246,0.2)" : "rgba(139,92,246,0.8)", color: isUser ? "#a78bfa" : "#fff", border: isUser ? "1px solid rgba(139,92,246,0.4)" : "none" }}>
      ...
      <div style={{ maxWidth: "72%", display: "flex", flexDirection: "column", gap: 8, alignItems: isUser ? "flex-end" : "flex-start" }}>
        <div style={{ padding: "10px 14px", fontSize: 13, lineHeight: 1.65, borderRadius: isUser ? "18px 4px 18px 18px" : "4px 18px 18px 18px", background: isUser ? "rgba(139,92,246,0.25)" : "rgba(255,255,255,0.06)", color: isUser ? "#e2e8f0" : "#cbd5e1", border: isUser ? "1px solid rgba(139,92,246,0.35)" : "1px solid rgba(255,255,255,0.08)" }}>
          {display.split("\n").map((line, i, arr) => (
...
function NexoraLogo() {
  return (
    <div style={{ position: "relative", width: 100, height: 100, margin: "0 auto 24px" }}>
      <div style={{ position: "absolute", inset: -15, borderRadius: "50%", background: "radial-gradient(circle, rgba(139,92,246,0.25) 0%, transparent 70%)", animation: "npulse 3s ease-in-out infinite" }} />
      <img src="/logoss.png" alt="Nexora AI" style={{ width: 100, height: 100, objectFit: "contain", filter: "drop-shadow(0 0 20px rgba(139,92,246,0.6))", animation: "npulse 3s ease-in-out infinite", position: "relative", zIndex: 1 }} />
    </div>
  );
}
...
export default function CareerChatbot({ userInfo }) {
  ...
  return (
    <>
      ...
      <div className="ncc" style={{ display: "flex", height: "100vh", width: "100vw", maxHeight: "100vh", background: "#09090f", fontFamily: "'DM Sans', sans-serif", color: "#e2e8f0", overflow: "hidden" }}>

        {/* ══ SIDEBAR ════════════════════════════════════════════ */}
        <aside style={{ width: 268, minWidth: 268, display: "flex", flexDirection: "column", background: "#0d0d16", borderRight: "1px solid rgba(255,255,255,0.06)" }}>

          {/* Logo */}
          <div style={{ padding: "18px 18px 14px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, flexShrink: 0, background: "linear-gradient(135deg,#7c3aed,#4f46e5)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, color: "#fff", fontFamily: "'Syne',sans-serif", boxShadow: "0 0 16px rgba(124,58,237,0.35)" }}>N</div>
              <div>
                <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Syne',sans-serif", letterSpacing: "-0.02em" }}>NEXOR<span style={{ color: "#7c3aed" }}>AI</span></p>
          ...
          <div style={{ padding: "0 10px 4px" }}>
            ...
            {MODES.map((m) => {
              ...
              return (
                <button key={m.id} onClick={() => setMode(m.id)} className="ncc-mode-btn"
          ...
          <div style={{ padding: "10px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "4px 4px 2px" }}>
              {userInfo?.imageUrl ? (
                <img src={userInfo.imageUrl} alt="" style={{ width: 28, height: 28, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} />
              ) : (
                <div style={{ width: 28, height: 28, borderRadius: "50%", flexShrink: 0, background: "rgba(139,92,246,0.2)", border: "1px solid rgba(139,92,246,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#a78bfa" }}>{userInfo?.initials || "ME"}</div>
              )}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, position: "relative", background: "#09090f" }}>
          ...
          <div style={{ position: "absolute", top: "15%", left: "50%", transform: "translateX(-50%)", width: 600, height: 600, borderRadius: "50%", pointerEvents: "none", zIndex: 0, background: "radial-gradient(circle, rgba(124,58,237,0.05) 0%, transparent 65%)" }} />

          {/* Topbar */}
          <div style={{ padding: "12px 24px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(13,13,22,0.85)", backdropFilter: "blur(12px)", position: "relative", zIndex: 10 }}>
            <div style={{ padding: "5px 12px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#94a3b8" }}>
          ...
          {/* Messages */}
          <div style={{ flex: 1, overflowY: "auto", padding: "20px 10%", display: "flex", flexDirection: "column", gap: 18, position: "relative", zIndex: 1 }}>
            {isEmpty && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, textAlign: "center", animation: "nfadeUp 0.5s ease" }}>
                <NexoraLogo />
                <h1 style={{ margin: "0 0 8px", fontSize: 30, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Syne',sans-serif", letterSpacing: "-0.03em" }}>
                  Good {greeting}, {userInfo?.name || "there"}.
                </h1>
                <p style={{ margin: "0 0 28px", fontSize: 17, color: "#64748b", fontWeight: 300 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, justifyContent: "center", maxWidth: 560 }}>
                  {chips.map((c) => (
                    <button key={c} onClick={() => send(c)} className="ncc-chip"
                      style={{ padding: "8px 16px", borderRadius: 20, fontSize: 12, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", color: "#94a3b8", cursor: "pointer", transition: "all 0.18s" }}>
                      {c}
                    </button>
                  ))}
                </div>
            ...
            {loading && (
              <div className="ncc-msg" style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                <div style={{ width: 30, height: 30, borderRadius: "50%", background: "rgba(139,92,246,0.8)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#fff", flexShrink: 0 }}>N</div>
            ...
            {!isEmpty && !loading && messages[messages.length - 1]?.role === "assistant" && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, paddingLeft: 42 }}>
                {chips.slice(0, 3).map((c) => (
                  <button key={c} onClick={() => send(c)} className="ncc-chip"
                    style={{ padding: "6px 14px", borderRadius: 20, fontSize: 11, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", color: "#64748b", cursor: "pointer", transition: "all 0.18s" }}>
                    {c}
                  </button>
                ))}
              </div>
          ...
          {/* Input */}
          <div style={{ padding: "14px 12%", borderTop: "1px solid rgba(255,255,255,0.06)", background: "rgba(13,13,22,0.92)", backdropFilter: "blur(12px)", position: "relative", zIndex: 10 }}>
            <div className="ncc-input-wrap" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 16, padding: "12px 14px", transition: "border-color 0.2s" }}>
              <textarea ref={textareaRef} rows={1} value={input}
                ...
                style={{ width: "100%", background: "transparent", border: "none", outline: "none", color: "#e2e8f0", fontSize: 14, resize: "none", lineHeight: 1.55, maxHeight: 120, overflow: "auto", fontFamily: "'DM Sans',sans-serif" }}
              />
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 10, paddingTop: 10, borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                  ...
                  <button onClick={handleUploadClick} disabled={uploading} className="ncc-upload"
                    style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 11px", borderRadius: 20, cursor: "pointer", fontSize: 11, fontWeight: 500, transition: "all 0.18s", background: uploadSuccess ? "rgba(16,185,129,0.09)" : "rgba(255,255,255,0.04)", border: uploadSuccess ? "1px solid rgba(16,185,129,0.28)" : "1px solid rgba(255,255,255,0.09)", color: uploadSuccess ? "#34d399" : "#64748b" }}>
                    {uploading
                      ? <span style={{ width: 11, height: 11, border: "2px solid rgba(139,92,246,0.3)", borderTopColor: "#7c3aed", borderRadius: "50%", display: "inline-block", animation: "nspin 0.8s linear infinite" }} />
                      : uploadSuccess
                      ? <svg width="11" height="11" viewBox="0 0 12 12" fill="none"><path d="M2 6l3 3 5-5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
                      : <svg width="11" height="11" viewBox="0 0 16 16" fill="none"><path d="M2 10v4h12v-4M8 2v9M5 5l3-3 3 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    }
                    {uploading ? "Uploading..." : uploadSuccess ? (uploadedFileName || "Resume attached") : "Upload Resume"}
                  </button>
                  {["Resume Review", "Skill Gap", "Mock Interview"].map((label) => {
                    const modeMap = { "Resume Review": "resume", "Skill Gap": "skillgap", "Mock Interview": "interview" };
                    const active  = mode === modeMap[label];
                    return (
                      <button key={label} onClick={() => setMode(modeMap[label])}
                        style={{ padding: "5px 11px", borderRadius: 20, fontSize: 11, cursor: "pointer", transition: "all 0.18s", background: active ? "rgba(139,92,246,0.14)" : "rgba(255,255,255,0.03)", border: active ? "1px solid rgba(139,92,246,0.32)" : "1px solid rgba(255,255,255,0.07)", color: active ? "#a78bfa" : "#475569", fontFamily: "'DM Sans',sans-serif" }}>
                        {label}
                      </button>
                    );
                  })}
                ...
                <button onClick={() => send()} disabled={!input.trim() || loading} className="ncc-send"
                  style={{ width: 34, height: 34, borderRadius: "50%", border: "none", cursor: "pointer", background: "#6d28d9", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.18s", flexShrink: 0, boxShadow: "0 0 14px rgba(109,40,217,0.35)" }}>
                  <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M2 8L14 2L8 14L7 9L2 8Z" fill="white"/></svg>
                </button>
              </div>
            </div>
            <p style={{ textAlign: "center", fontSize: 10, color: "#1e293b", margin: "8px 0 0" }}>Nexora Career AI</p>
          </div>
        </div>
      </div>
    </>
